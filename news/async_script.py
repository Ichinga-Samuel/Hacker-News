...
async def check_item(item_id, model):
    ...
    loop = asyncio.get_running_loop()
    try:
        item = await loop.run_in_executor(None, lambda: model.objects.get(id=item_id))  #model.objects.get(id=item_id)
...
async def check_id(id_):
    ...
    :param id_:
    :return: True if the item is existing the model it belongs to and the item else false and None
    """
